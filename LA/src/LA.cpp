...
#include "LA.h"
#ifdef LA_DEBUG
#define DEBUG_OUT (std::cerr << "DEBUG-LA: ") // or any other ostream
#else
...
namespace LA {
    ...
    ItemFName print_FName(LA::print_str);
    ItemFName input_FName(LA::input_str);
    ItemFName tensor_FName(LA::tensor_str);
    ...
    bool isTerminator(Instruction * inst) {
        return  inst->type == InstType::inst_ret
            ||  inst->type == InstType::inst_ret_var
            ||  inst->type == InstType::inst_branch
            ||  inst->type == InstType::inst_branch_cond;
